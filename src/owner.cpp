...
void OwnerSelectData(sqlite3* db, std::string surname) {
	...
	const char* sql = "Select O.surname as owner_surname, R.date as date_of_race, H.name as horse_name, R.taken_place\n"
		"from Races R\n"
...
void OwnerSelectBestHorse(sqlite3* db, std::string surname) {
	...
	const char* sql = "SELECT h.name as horse_name, h.age as horse_age, h.experience as horse_experience, h.paid_price as horse_price"
		", owner_id as horse_owner_id, r.date as race_date, j.id as jockey_id, j.surname as jockey_surname"
		", j.experience as jockey_experience, j.birth_date as jockey_birth_date, j.address as jockey_address\n"
		"FROM Horses h\n"
