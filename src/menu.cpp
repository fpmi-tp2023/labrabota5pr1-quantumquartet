...
void SignIn(sqlite3* db) {
	...
	if (rc == SQLITE_OK) {
		...
		sqlite3_bind_text(res, 2, password.c_str(), password.length(), SQLITE_TRANSIENT);
	}
	else {
	...
	if (step != SQLITE_ROW) {
		...
		user_type=std::string(reinterpret_cast<const char*>(
			sqlite3_column_text(resType, 0)
			));
...
void SignUp(sqlite3* db) {
	...
	if (step != SQLITE_ROW) {
		...
		sql = "INSERT INTO users (surname, password, user_type_id) VALUES (?, ?, ?);";
