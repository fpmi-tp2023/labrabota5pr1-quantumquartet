...
void JockeyAllRaces(sqlite3* db, std::string surname) {
    const char* sql =
        "Select J.surname as jockey_surname,H.name as horse_name,R.date as "
        "date_of_race,R.taken_place\n"
        "from Races R\n"
...
void JockeyRacesInInterval(sqlite3* db, std::string surname) {
    ...
    const char* sql =
        "Select  R.jockey_id,J.surname as jockey_surname,"
        "J.experience as jockey_experience,J.birth_date as jockey_birth_date,"
        "j.address as jockey_address,"
        "R.id as race_id, R.date as race_date, R.number_of_race, R.horse_id, "
        "R.taken_place\n"
        "from Races R\n"
        "inner join Horses H on R.horse_id = H.id\n"
        "inner join Jockeys j on R.jockey_id = J.id\n"
    ...
    if (rc == SQLITE_OK) {
        sqlite3_bind_text(res, 1, surname.c_str(), surname.length(),
            SQLITE_TRANSIENT);
        sqlite3_bind_text(res, 2, intervalBegin.c_str(), intervalBegin.length(),
            SQLITE_TRANSIENT);
        sqlite3_bind_text(res, 3, intervalEnd.c_str(), intervalEnd.length(),
            SQLITE_TRANSIENT);
    }
    else {
        fprintf(stderr, "Failed to execute statement: %s\n", sqlite3_errmsg(db));
